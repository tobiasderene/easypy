...
export default function LandingPage() {
  ...
  return (
    <div style={{ fontFamily: "'Nunito', sans-serif", background: "#f8fafc", color: "#111827", overflowX: "hidden" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .nav-link { background: none; border: none; cursor: pointer; font-family: 'Nunito', sans-serif; font-size: 14px; font-weight: 600; color: #374151; transition: color 0.2s; padding: 8px 12px; border-radius: 8px; }
        .nav-link:hover { color: ${BRAND}; }

        .btn-primary { background: ${BRAND}; color: white; border: none; border-radius: 10px; font-family: 'Nunito', sans-serif; font-weight: 700; cursor: pointer; transition: all 0.2s; }
        .btn-primary:hover { background: ${BRAND_DARK}; transform: translateY(-2px); box-shadow: 0 8px 24px rgba(5,110,183,0.25); }

        .btn-outline { background: white; color: ${BRAND}; border: 2px solid ${BRAND}; border-radius: 10px; font-family: 'Nunito', sans-serif; font-weight: 700; cursor: pointer; transition: all 0.2s; }
        .btn-outline:hover { background: ${BRAND}; color: white; transform: translateY(-2px); }

        .benefit-card { background: white; border-radius: 16px; padding: 28px 24px; border: 1.5px solid #e5e7eb; transition: all 0.3s; }
        .benefit-card:hover { border-color: ${BRAND}; box-shadow: 0 8px 32px rgba(5,110,183,0.1); transform: translateY(-4px); }

        .product-card { background: white; border-radius: 16px; overflow: hidden; border: 1.5px solid #e5e7eb; transition: all 0.3s; }
        .product-card:hover { box-shadow: 0 8px 32px rgba(5,110,183,0.12); transform: translateY(-4px); border-color: ${BRAND}; }

        .stat-item { text-align: center; padding: 24px 16px; }

        @keyframes float { 0%,100% { transform: translateY(0); } 50% { transform: translateY(-12px); } }
        @keyframes pulse-ring { 0% { transform: scale(0.95); opacity: 0.7; } 70% { transform: scale(1.1); opacity: 0; } 100% { transform: scale(1.1); opacity: 0; } }
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "80px 24px", width: "100%" }}>
          <div className="hero-grid" style={{ display: "flex", alignItems: "center", gap: 64 }}>
            <div style={{ flex: 1 }}>
              ...
              <p className={`fade-up d2 ${heroVisible ? "visible" : ""}`} style={{ fontSize: 18, color: "#475569", lineHeight: 1.7, marginBottom: 36, maxWidth: 520 }}>
              ...
              <div className={`cta-btns fade-up d3 ${heroVisible ? "visible" : ""}`} style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
              ...
              <div style={{ position: "relative", animation: "float 4s ease-in-out infinite" }}>
                <div style={{ width: 340, height: 420, background: "white", borderRadius: 28, boxShadow: "0 32px 80px rgba(5,110,183,0.18)", overflow: "hidden", border: "1.5px solid #e5e7eb" }}>
                  ...
                  <div style={{ padding: "20px 24px" }}>
                    ...
                    <div style={{ marginTop: 16, background: BRAND, color: "white", borderRadius: 10, padding: "10px", textAlign: "center", fontSize: 14, fontWeight: 700 }}>
                      → Comprar
                    </div>
                  </div>
                </div>
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div className="about-grid" style={{ display: "flex", gap: 80, alignItems: "center" }}>
            ...
            <div className={`fade-up d2 ${aboutVisible ? "visible" : ""}`} style={{ flex: 1 }}>
              <div className="stats-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 2, background: "#e5e7eb", borderRadius: 20, overflow: "hidden" }}>
                {stats.map((s, i) => (
          ...
          <div className={`fade-up ${ctaVisible ? "visible" : ""}`}>
            ...
            <div className="cta-btns" style={{ display: "flex", gap: 14, justifyContent: "center", flexWrap: "wrap" }}>
              <button onClick={() => window.location.href="/signup"}
                style={{ background: "white", color: BRAND, border: "none", borderRadius: 12, padding: "15px 36px", fontSize: 16, fontWeight: 800, cursor: "pointer", transition: "all 0.2s", fontFamily: "'Nunito', sans-serif" }}
              ...
              <button onClick={() => window.location.href="/login"}
                style={{ background: "transparent", color: "white", border: "2px solid rgba(255,255,255,0.5)", borderRadius: 12, padding: "15px 28px", fontSize: 16, fontWeight: 700, cursor: "pointer", transition: "all 0.2s", fontFamily: "'Nunito', sans-serif" }}
